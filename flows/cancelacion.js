...
const { getUserData,cancelarPedido } = require('./basededatos.js');
...
const flowCancelacion = addKeyword('7', { sensitive: true })
    ...
    .addAnswer('Ingresa la talla que enviaste como la ingresaste cuando hiciste la compra:', { capture: true }, async (ctx, { state }) => {
        await state.update({ talla: ctx.body });
        console.log('Talla capturada:', state.getMyState().talla);
    })
    .addAnswer('Ingresa el color que enviaste como lo ingresaste cuando hiciste la compra:', { capture: true }, async (ctx, { state, flowDynamic }) => {
